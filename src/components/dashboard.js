...
import Continental2 from '../continental2.jpg';
import Continental3 from '../continental3.jpg';
import native from '../native.jpg';
import native2 from '../native2.jpeg';
import native3 from '../native3.jpg';
import native4 from '../native4.jpg';
import pastries from '../pastries.jpg';
import pastries2 from '../patries2.jpeg';
...
class Dashboard extends Component{
 ...
 render(){
  ...
  const pics = (data) => {
   ...
   if (data === 'Continental Dishes' ){return conti[ran(conti)]}
   else if (data === 'Native Dishes' ){return nati[ran(nati)]} 
   else if (data === 'Pastries' ){return pastri[ran(pastri)]} 
  } 
  ...
  const chefs = this.state.chefs.map(chef => <div key={chef.id} className="content col-md-12 row">
    ...
    <div><span> Company Name: </span> {chef.company_name}</div>
      <div> <span> Specialty: </span>{chef.specialty}</div>
      <div> <span> Location: </span>{chef.location}</div>
    </div>
  </div>)
    return(
